...
def simulate_llm_response(scene_id, file_id, lock, llm_response, eval_stat_path, task_names):
    ...
    report = evaluate_task(vocab_path, scene_id, file_id, llm_response)
    with lock:
...
def worker_task(queue, lock, eval_stat_path, task_names):
    while True:
        task = queue.get()
        if task is None:
            break
...
def simulate_one_llm(llm_response_path, worker_num: int=5, result_dir: str='./results'):
    manager = Manager()
    lock = manager.Lock()
    ...
    if worker_num > 1:
        ...
        for i in range(worker_num):
            worker = Process(target=worker_task, args=(task_queue, lock, eval_stat_path, task_names))
            worker.start()
